...
def neg_log_lik_gp(hyps, x, y, k, jitter=1e-3):
    N, dim_input = x.shape
    _, dim_output = y.shape
    K = k(hyps, x, x) + np.eye(N)*jitter
    K_chol = np.linalg.cholesky(K)
    y_per = np.expand_dims(np.transpose(y, (1, 0)), axis=-1)
    y_scaled = np.linalg.solve(K_chol, y_per)[...,-1]
    sign_K, logdet_K = np.linalg.slogdet(K)
    log_lik = -0.5*np.sum(y_scaled ** 2) - 0.5*logdet_K*dim_output
    return -log_lik


def kernel_hyper_estimation(x, y, k, hyps0=None):
    N, dim_input = x.shape
    _, dim_output = y.shape
    if hyps0 is None:
        hyps0 = np.zeros(k()[0])
    res = minimize(neg_log_lik_gp, hyps0, method='L-BFGS-B', options={'disp': True}, args=(x, y, k))
    return res.x
